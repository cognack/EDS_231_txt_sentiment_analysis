...
neutral <- length(which(tweets_sent$sent_score == "NaN"))
...
Future Things to Check Out
 - `quanteda.textplots` - for better plots and text analysis with quanteda
...
# create 'document feature matrix' (from 'quantata')
# features are words
...
hash_tib %>%
   count(term) %>%
   with(wordcloud(term, n, max.words = 100, random.color = FALSE))
```
